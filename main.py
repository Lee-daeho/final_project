...
from config import *
from savefigure import top_worst, labeled_savecam, unlabeled_savecam
...
parser.add_argument("-p","--spl", type=bool, default=False,
                    help="Training on the entire dataset")
parser.add_argument("-rd", "--random_start", type=bool ,default=False,
                    help="Initial dataset random or not")
...
if __name__ == '__main__':
    ...
    methods = ['Random', 'UncertainGCN', 'CoreGCN', 'CoreSet', 'lloss','VAAL', 'daeho', 'cam', 'CAAL']
    ...
    '''
    results = open('results_'+str(args.method_type)+"_"+args.dataset +'_main'+str(args.cycles)+str(args.total)+'_'+str(args.cycles)+'cycles.txt','w')
    ...
    for trial in range(TRIALS):
        ...
        if not args.random_start:
            ...
            if not os.path.exists(args.dataset + '_initial_indices.pkl'):
                random.shuffle(indices)
                with open(args.dataset + '_initial_indices.pkl', 'wb') as f:
                    pickle.dump(indices, f)
            else:
                with open(args.dataset + '_initial_indices.pkl', 'rb') as f:
        ...
        for cycle in range(CYCLES):
            ...
            if not method == 'daeho' and not method == 'jundoo':
                ...
                if method == "CAAL":
                    arg, labeled_graycam, unlabeled_graycam = query_samples(models, method, data_unlabeled, subset, labeled_set, cycle, args)
                else:
                    arg = query_samples(models, method, data_unlabeled, subset, labeled_set, cycle, args)
                    
                ## save figure
                #top_worst(arg, data_train, subset, method, models, trial, cycle)
                
                #labeled_savecam(data_train, labeled_graycam, labeled_set, trial, cycle, NO_CLASSES)
                #unlabeled_savecam(data_train, unlabeled_graycam, subset, trial, cycle, NO_CLASSES, arg)
                ...
                if cycle == (CYCLES-1):
                    ...
                    print("Finished.")
                    break
                arg, vae = query_samples(models, method, data_unlabeled, subset, labeled_set, cycle, args)

                acc = test(vae, EPOCH, method, dataloaders, mode='test')
                print('Trial {}/{} || Cycle {}/{} || Label set size {}: Test acc {}'.format(trial+1, TRIALS, cycle+1, CYCLES, len(labeled_set), acc))
                np.array([method, trial+1, TRIALS, cycle+1, CYCLES, len(labeled_set), acc]).tofile(results, sep=" ")
                results.write("\n")
            ...
            if not spl:
                ...
                v = int(acc/100 * SUBSET)
                print('v : ',v)
                labeled_set += list(torch.tensor(subset)[arg][v:v+ADDENDUM].numpy())
                listd = list(torch.cat([torch.tensor(subset)[arg][:v], torch.tensor(subset)[arg][v+ADDENDUM:]]).numpy())
                unlabeled_set = listd + unlabeled_set[SUBSET:]
                print(len(labeled_set), min(labeled_set), max(labeled_set))
                # Create a new dataloader for the updated labeled dataset
                dataloaders['train'] = DataLoader(data_train, batch_size=BATCH,
