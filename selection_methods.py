...
# Custom
from config import *
from models.query_models import VAE, Discriminator, GCN, CVAE
from data.sampler import SubsetSequentialSampler
from kcenterGreedy import kCenterGreedy
from load_dataset import CamDataset
...
def train_cvaal(models, optimizers, labeled_dataloader, unlabeled_dataloader, cycle):
    vae = models['vae']
    discriminator = models['discriminator']
    vae.train()
    discriminator.train()
    ...
    for iter_count in range(train_iterations):
        ...
        # VAE step
        for count in range(num_vae_steps):  # num_vae_steps
            recon, _, mu, logvar, _ = vae(labeled_imgs)
            unsup_loss = vae_loss(labeled_imgs, recon, mu, logvar, beta)
            unlab_recon, _, unlab_mu, unlab_logvar, _ = vae(unlabeled_imgs)
            transductive_loss = vae_loss(unlabeled_imgs,
                                         unlab_recon, unlab_mu, unlab_logvar, beta)
        ...
        for count in range(num_adv_steps):
            with torch.no_grad():
                _, _, mu, _, _ = vae(labeled_imgs)
                _, _, unlab_mu, _, _ = vae(unlabeled_imgs)
...
def get_camresult(model, data_loader, num_cls):
    ...
    for i, (input, target, _) in enumerate(data_loader):  # calculate grayscale cam
        ...
        for cls in range(num_cls):
            ...
            gray_img = torch.mean(input, dim=1).unsqueeze(1)
